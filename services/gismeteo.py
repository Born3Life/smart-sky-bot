...
def fetch_forecast(city: str) -> list[dict[str, Any]] | None:
    """Extract multi-day forecast from Gismeteo page."""
    url = _city_url(city)
    if url is None:
        return None
    ...
    try:
        ...
        logger.warning("gismeteo forecast fetch failed for %s", city)
        return None

    text = resp.text

    # Try extracting temperature data from the weather-table rows
    import re

    entries: list[dict[str, Any]] = []
    today = datetime.now().strftime("%Y-%m-%d")

    # Look for date+temp patterns in the page JSON
    data = _extract_json_block(text, "cw")
    if not data:
        return None
    ...
    # Try to extract date arrays
    dates_raw = cw.get("date", [])
    if not isinstance(dates_raw, list):
        return None

    temps_day = cw.get("temperatureAir", [])
    if not isinstance(temps_day, list):
        temps_day = []

    temps_night = cw.get("temperatureNight", [])
    if not isinstance(temps_night, list):
        temps_night = []

    icons = cw.get("iconWeather", [])
    if not isinstance(icons, list):
        icons = []

    for i in range(max(len(dates_raw), 7)):
        date_str = dates_raw[i] if i < len(dates_raw) else ""
        if not date_str:
            continue
        try:
        ...
        day_t = float(temps_day[i]) if i < len(temps_day) else 0
        night_t = float(temps_night[i]) if i < len(temps_night) else day_t - 5
        icon = str(icons[i]) if i < len(icons) else "c"

        entries.append({
            "date": date_str[:10],
            "temp": round(day_t),
            "temp_night": round(night_t),
            "feels_like": round(day_t),
            "humidity": 50,
            "wind": 0,
            "desc": _description(icon),
            "icon": _gismeteo_icon(icon),
            "pressure": 1013,
            "clouds": 0,
            "rain": 0,
            "snow": 0,
